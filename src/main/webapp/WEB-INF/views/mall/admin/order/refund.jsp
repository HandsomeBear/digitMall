	...
	<body>
						...
						<tbody>
							<c:forEach var="order" items="${orders }">
								<tr>
									...
									<td style="width:360px;">${order.title }</td>
									<td style="width:40px;">${order.color }</td>
									<td style="width:50px;">${order.version }</td>
									<td style="width:30px;">${order.quantity }</td>
									<td style="width:30px;"><a href="javascript:void(0)" onclick="refund('${order.order_no }')">退款</a></td>
								</tr>
							</c:forEach>
						</tbody>
					</table>
		...
		<script type="text/javascript">
			...
			function check(operation){
				...
				if(operation=="prev"){
					if(parseInt(parseInt(currentPage)-1)==0){
						...
						window.location.href="admin/torefund?currentPage="+parseInt(parseInt(currentPage)-1);
					}
					...
					if(parseInt(totalPage)==0){
						...
						window.location.href="admin/torefund?currentPage=1";
					}
				}else if(operation=="last"){
					if(parseInt(totalPage)==0){
						alert("There is no data!");
					}else{
						window.location.href="admin/torefund?currentPage="+totalPage;
					}
					...
					window.location.href="admin/torefund?currentPage="+currentPage;
				}
			}
			...
			function refund(orderno){
				var data = {};
				data["orderno"] = orderno;
				var result = ajaxRequest("admin/refund", "text", JSON.stringify(data));
				if(result=="SUCCESS"){
					alert("Status Change Success!");
					window.location.href="admin/torefund?currentPage=1";
				}else{
					alert("Status Change Fail!");
					window.location.href="admin/torefund?currentPage=1";
				}
			}
		</script>
